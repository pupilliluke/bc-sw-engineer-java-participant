...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const value = useMemo<AuthValue>(
    () => ({
      ...
      logout: () => {
        // Stateless bearer tokens, the API has no revoke endpoint, so the token
        // is discarded here and the workspace unmounts with its customer state.
        tokenStore.clear()
        setState({ status: 'anonymous' })
        console.log('logout', 'lab-request-001')
      },
    }),
    [state]
  )
