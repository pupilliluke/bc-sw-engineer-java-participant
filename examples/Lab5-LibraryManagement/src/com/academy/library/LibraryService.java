...
public class LibraryService {
    ...
    public void runPerformanceComparison() {
        final int elementCount = 100_000;
        final int frontInsertCount = 50_000;
        final int accessCount = 10_000;

        System.out.println("ArrayList vs LinkedList performance (times in ms)");
        System.out.printf("%-24s | %10s | %10s%n", "Operation", "ArrayList", "LinkedList");
        System.out.println("-------------------------|------------|------------");
        ...
        double arrayAppend = toMillis(System.nanoTime() - start);
        ...
        double linkedAppend = toMillis(System.nanoTime() - start);
        printTimingRow("Add at end", arrayAppend, linkedAppend);

        // 2. Insert at the front (forces ArrayList to shift every element)
        List<Integer> arrayFront = new ArrayList<>();
        start = System.nanoTime();
        for (int i = 0; i < frontInsertCount; i++) {
            arrayFront.add(0, i);
        }
        double arrayFrontInsert = toMillis(System.nanoTime() - start);

        List<Integer> linkedFront = new LinkedList<>();
        start = System.nanoTime();
        for (int i = 0; i < frontInsertCount; i++) {
            linkedFront.add(0, i);
        }
        double linkedFrontInsert = toMillis(System.nanoTime() - start);
        printTimingRow("Insert at front", arrayFrontInsert, linkedFrontInsert);

        // 3. Random access by index, spread across the whole list
        int step = elementCount / accessCount;
        start = System.nanoTime();
        for (int i = 0; i < accessCount; i++) {
            arrayList.get(i * step);
        }
        double arrayAccess = toMillis(System.nanoTime() - start);

        start = System.nanoTime();
        for (int i = 0; i < accessCount; i++) {
            linkedList.get(i * step);
        }
        double linkedAccess = toMillis(System.nanoTime() - start);
        printTimingRow("Random access (by index)", arrayAccess, linkedAccess);

        System.out.println();
        System.out.println("ArrayList wins on end-append and index access;");
        System.out.println("LinkedList wins on front inserts (no element shifting).");
    }
    ...
    private void printTimingRow(String label, double arrayMs, double linkedMs) {
        System.out.printf("%-24s | %7.2f ms | %7.2f ms%n", label, arrayMs, linkedMs);
    }
