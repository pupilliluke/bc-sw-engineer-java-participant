...
import com.northstar.crm.logging.CorrelationFilter;
...
public class CustomerController {
    ...
    public ResponseEntity<Customer> create(@RequestBody Customer body) {
        ...
        if (body.getName() == null || body.getName().isBlank()) {
            log.warn("Rejecting create reason=missing_full_name customerId={}", body.getId());
            throw new IllegalArgumentException("fullName required");
        }
        if (body.getStatus() == null || body.getStatus().isBlank()) {
            log.warn("Rejecting create reason=missing_status customerId={}", body.getId());
            throw new IllegalArgumentException("status required");
        }
        Customer created = customers.create(body, MDC.get(CorrelationFilter.MDC_KEY));
