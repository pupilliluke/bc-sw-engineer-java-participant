...
2. **File → Open…** → `~/java-bootcamp` (Lab 0 workspace root — same folder every lab).  
   If `examples/lab35-crm` does not exist yet, create it as the lab GUIDE describes; keep the workspace open at `~/java-bootcamp`.
...
6. **View → Tool Windows → Terminal** → `cd ~/java-bootcamp` then `cd examples/lab35-crm/crm-ui` when ready.
...
3. **Terminal → New Terminal** → `cd examples/lab35-crm/crm-ui` for this lab’s commands.
...
| Evidence / screenshots | `~/java-bootcamp/notes/screenshots/lab-35` |
...
Save screenshots under `~/java-bootcamp/notes/screenshots/lab-35` (Lab 0 workspace layout). Capture IntelliJ (project tree + Run/Terminal) on macOS. Redact passwords, tokens, and kubeconfig contents.
