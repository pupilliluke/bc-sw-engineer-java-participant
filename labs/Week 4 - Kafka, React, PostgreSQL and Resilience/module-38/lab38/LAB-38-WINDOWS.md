...
2. **File → Open…** → `%USERPROFILE%\java-bootcamp` (Lab 0 workspace root — same folder every lab).  
   If `examples\lab38-crm` does not exist yet, create it as the lab GUIDE describes; keep the workspace open at `%USERPROFILE%\java-bootcamp`.
...
6. **View → Tool Windows → Terminal** (PowerShell) → `cd $env:USERPROFILE\java-bootcamp` then `cd examples\lab38-crm` when ready.
...
3. **Terminal → New Terminal** (PowerShell) → `cd examples\lab38-crm` for this lab’s commands.
...
Save screenshots under `%USERPROFILE%\java-bootcamp\notes\screenshots\lab-38` (Lab 0 workspace layout). Capture IntelliJ (project tree + Run/Terminal) on Windows. Redact passwords, tokens, and kubeconfig contents.
