...
2. **File → Open…** → `~/java-bootcamp` (Lab 0 workspace root — same folder every lab).  
   If `examples/lab39-crm` does not exist yet, create it as the lab GUIDE describes; keep the workspace open at `~/java-bootcamp`.
3. Trust the project if prompted.
4. **File → Project Structure → Project** → SDK = **21**, language level **21**.
5. Maven labs: open the `pom.xml` under `examples/lab39-crm` so IntelliJ imports the project; wait for indexing.
...
3. **Terminal → New Terminal** → `cd examples/lab39-crm` for this lab’s commands.
...
Save screenshots under `~/java-bootcamp/notes/screenshots/lab-39` (Lab 0 workspace layout). Capture IntelliJ (project tree + Run/Terminal) on macOS. Redact passwords, tokens, and kubeconfig contents.
...
| 5 | Screenshots (if required) saved under `notes/screenshots/lab-39/` | Pass / Fail |
