...
2. **File → Open…** → `~/java-bootcamp` (Lab 0 workspace root — same folder every lab).  
   If `examples/lab34-crm` does not exist yet, create it as the lab GUIDE describes; keep the workspace open at `~/java-bootcamp`.
...
6. **View → Tool Windows → Terminal** → `cd ~/java-bootcamp` then `cd examples/lab34-crm/crm-ui` when ready.
...
3. **Terminal → New Terminal** → `cd examples/lab34-crm/crm-ui` for this lab’s commands.
...
| Evidence / screenshots | `~/java-bootcamp/notes/screenshots/lab-34` |
...
1. Open the SPA under `examples/lab34-crm/crm-ui` (or `frontend` for Lab 50).
...
Save screenshots under `~/java-bootcamp/notes/screenshots/lab-34` (Lab 0 workspace layout). Capture IntelliJ (project tree + Run/Terminal) on macOS. Redact passwords, tokens, and kubeconfig contents.
...
| 2 | Lab project under `examples/lab34-crm` as in [LAB-34-GUIDE.md](LAB-34-GUIDE.md) | Pass / Fail |
...
| 5 | Screenshots (if required) saved under `notes/screenshots/lab-34/` | Pass / Fail |
