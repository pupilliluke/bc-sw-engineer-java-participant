...
2. **File → Open…** → `~/java-bootcamp` (Lab 0 workspace root — same folder every lab).  
   If `examples/lab8-crm` does not exist yet, create it as the lab GUIDE describes; keep the workspace open at `~/java-bootcamp`.
...
3. **Terminal → New Terminal** → `cd examples/lab8-crm` for this lab’s commands.
...
| Evidence / screenshots | `~/java-bootcamp/notes/screenshots/lab-8` |
...
mkdir -p notes/screenshots/lab-8
...
Save screenshots under `~/java-bootcamp/notes/screenshots/lab-8` (Lab 0 workspace layout). Capture IntelliJ (project tree + Run/Terminal) on macOS. Redact passwords, tokens, and kubeconfig contents.
