...
5. Maven labs: open the `pom.xml` under `examples/customer-management-platform` so IntelliJ imports the project; wait for indexing.
...
3. **Terminal → New Terminal** → `cd examples/customer-management-platform` for this lab’s commands.
...
Save screenshots under `~/java-bootcamp/notes/screenshots/lab-49` (Lab 0 workspace layout). Capture IntelliJ (project tree + Run/Terminal) on macOS. Redact passwords, tokens, and kubeconfig contents.
...
| 2 | Lab project under `examples/customer-management-platform` as in [LAB-49-GUIDE.md](LAB-49-GUIDE.md) | Pass / Fail |
...
| 5 | Screenshots (if required) saved under `notes/screenshots/lab-49/` | Pass / Fail |
